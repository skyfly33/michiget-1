...
//		   $("input[name=v_searchField]").val("title");
		...
		//   $("input[name=v_searchRange]").val("365");
		   ...
		   $("input[name=v_chartType]").val("flash");
...
//		if($("#v_userGubun").val()=='01'){
//			searchType=	["newTurn","hanaPortal","guideRule","hanaTv","newTurnCoP"];
//		}else{
//			searchType=	["prodPlus","newTurn","hanaPortal","guideRule","newTurnCoP"];
//			
//		}
		...
		if($("#category0").text()!='-'&&$("#category0").text()=='0'){//전체검색결과가 없을 경우
			...
			if($("#category1").text()!='-'&&$("#category1").text()=='0'){//뉴턴 검색결과 없을 경우
				if(userGubun=='00'||userGubun=='01'){
			...
			if($("#category2").text()!='-'&&$("#category2").text()=='0'){//그룹웨어 검색결과 없을 경우
				if(userGubun=='00'||userGubun=='01'){
					tempDiv="#"+searchType[1]+"ResultDiv";
					$(tempDiv).html('').hide();
					if($("input[name=v_searchCategory]").val()==searchType[1]){
			...
			if($("#category3").text()!='-'&&$("#category3").text()=='0'){//내규/업무편람 검색결과 없을 경우
				if(userGubun=='00'||userGubun=='01'){
					tempDiv=	"#"+searchType[2]+"ResultDiv";
					$(tempDiv).html('').hide();
					if($("input[name=v_searchCategory]").val()==searchType[2]){
			...
			if($("#category4").text()!='-'&&$("#category4").text()=='0'){//사이버 학당 검색결과 없을 경우
			...
			if($("#category5").text()!='-'&&$("#category5").text()=='0'){//Cop 검색결과 없을 경우
			...
			if($("#category6").text()!='-'&&$("#category6").text()=='0'){//상품플러스 검색결과 없을 경우
			...
			if($("#category7").text()!='-'&&$("#category7").text()=='0'){//은행 자산관리 서양식 검색결과 없을 경우
	...
	function checkSearchType(){
		...
		if($("#v_userGubun").val()=='00'||$("#v_userGubun").val()=='01'){
			searchType=	["newTurn","hanaPortal","guideRule","hanaTv","newTurnCoP","bankform"];
		}else{
			searchType=	["prodPlus","newTurn","hanaPortal","guideRule","newTurnCoP"];
	...
	function totalSearch(){
		...
		if($("#timeLineDiv").css('display')!='none'){
			//piSearchAjax('timeLine','flash');// 시계열 분석용 Flash or Chart
		}
		...
		/*
		// 검색 값이 있다면 실행
		if(jQuery("#v_query").val() != ""){
			// urlManager.js의 내가 찾은 검색어 등록	
			setTimeout(function(){
				myFindKeyword.myKeywordUpdate();	
			}, 500);
	...
	function changeTotalCountResult(subCategory){
		if(subCategory=='all'){
				$('#mainContentsCnt').text(tspTools.addCommas($("#category0").text()));
		}else if(subCategory=='newTurn'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category1").text()));
		}else if(subCategory=='kms_map'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category11").text()));
		}else if(subCategory=='kms_qna'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category12").text()));
		}else if(subCategory=='kms_faq'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category13").text()));
		}else if(subCategory=='kms_wiki'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category14").text()));
		}else if(subCategory=='hanaPortal'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category2").text()));
		}else if(subCategory=='approval'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category21").text()));
		}else if(subCategory=='gwboard'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category22").text()));
		}else if(subCategory=='guideRule'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category3").text()));
		}else if(subCategory=='rule'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category31").text()));
		}else if(subCategory=='guide'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category32").text()));
		}else if(subCategory=='hanaTv'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category4").text()));
		}else if(subCategory=='newTurnCoP'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category5").text()));
		}else if(subCategory=='prodPlus'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category6").text()));
		}else if(subCategory=='prodInfo'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category61").text()));
		}else if(subCategory=='prodBoard'){
			$('#mainContentsCnt').text(tspTools.addCommas($("#category62").text()));
		}else if(subCategory=='bankform'){
	...
	function getCategoryText(subCategory){
		var _text='';
		if(subCategory=='prodPlus') _text='상품플러스';
		else if(subCategory=='prodInfo') _text='상품정보';
		else if(subCategory=='prodBoard') _text='게시글';
		else if(subCategory=='newTurn') _text='newTURN';
		else if(subCategory=='kms_map') _text='지식맵';
		else if(subCategory=='kms_qna') _text='Q&A';
		else if(subCategory=='kms_faq') _text='FAQ';
		else if(subCategory=='kms_wiki') _text='위키북';
		else if(subCategory=='hanaPortal') _text='하나포탈';
		else if(subCategory=='approval') _text='전자결재문서';
		else if(subCategory=='gwboard') _text='게시판';
		else if(subCategory=='guideRule') _text='내규/편람';
		else if(subCategory=='rule') _text='내규/편람';
		else if(subCategory=='guide') _text='업무편람';
		else if(subCategory=='hanaTv') _text='사이버학당 하나TV';
		else if(subCategory=='newTurnCoP') _text='newTURN COP';
		else if(subCategory=='bankform') _text='은행자산 서양식';
		return _text;
	}
			...
			$("div#s_SC").find('span.sc_'+i).each(function (){
				...
				if(_type=='gubun'&&i==1){//업무구분 선택 해제시
					...
					if(category=='custom'&&subCategory=='365'){
						...
						goDateRangeResults('custom','365');//default 1년전
					}
	...
	function piCategoryCountAjax(){
		...
		hideNoResult();
		jQuery.post(
				"/piSearch.do?cmd=categoryCountSearch",(
						$("#searchGNB").serialize()
						),
						function (data,textStatus){
						if(textStatus == "success"){
							if(data.category0!=undefined){//전체검색결과가 있을경우
								...
								$("#category0").text(tspTools.addCommas(data.category0));
									...
									if(data.category1!=undefined){
										...
										if(data.category11!=undefined){//newTurn 지식맵 검색결과
											$("#category11").text(tspTools.addCommas(data.category11));
											$("#rightClusterDiv").show();//newTurn 지식맵 검색결과가 있으면오른쪽 클러스터링 쪽 영역 보여주기
												clusterSearchAjax('clusterTagMini');
											if($("#cluster_flex").css("display")=='block'){
												
												clusterSearchAjax(clusterMethod);// 클러스터 검색결과
											}
										}else{
											$("#category11").text('0');
																						
										}
										if(data.category12!=undefined){//newTurn Q&A 검색결과
											$("#category12").text(tspTools.addCommas(data.category12));
										}else{
											$("#category12").text('0');
										}
										if(data.category13!=undefined){//newTurn FAQ 검색결과
											$("#category13").text(tspTools.addCommas(data.category13));
										}else{
											$("#category13").text('0');
										}
										if(data.category14!=undefined){//newTurn Wiki 검색결과
											$("#category14").text(tspTools.addCommas(data.category14));
										}else{
											$("#category14").text('0');
										}
									}else{
									...
									if(data.category2!=undefined){
										...
										if(data.category21!=undefined){//포탈 전자결재 검색결과
											$("#category21").text(tspTools.addCommas(data.category21));
										}else{
											$("#category21").text('0');
										}
										if(data.category22!=undefined){//포탈 게시판 검색결과
											$("#category22").text(tspTools.addCommas(data.category22));
										}else{
											$("#category22").text('0');
										}
									}else{
									...
									if(data.category3!=undefined){//내규/편람 검색결과
										$("#category3").text(tspTools.addCommas(data.category3));
										if($("#v_userGubun").val()=='00'||$("#v_userGubun").val()=='01'){
											if(data.category31!=undefined){//내규 검색결과
												$("#category31").text(tspTools.addCommas(data.category31));
											}else{
												$("#category31").text('0');
											}
											if(data.category32!=undefined){//업무편람 검색결과
												$("#category32").text(tspTools.addCommas(data.category32));
											}else{
												$("#category32").text('0');
											}
										}else{
											if(data.category33!=undefined){//사규(대투) 검색결과
												$("#category31").text(tspTools.addCommas(data.category33));
											}else{
												$("#category31").text('0');
											}
											if(data.category34!=undefined){//업무편람(대투) 검색결과
												$("#category32").text(tspTools.addCommas(data.category34));
											}else{
												$("#category32").text('0');
											}
										}
									...
									if(data.category4!=undefined){// 하나TV
										$("#category4").text(tspTools.addCommas(data.category4));
									}else{
										$("#category4").text('0');
									}
									if(data.category5!=undefined){// COP
										$("#category5").text(tspTools.addCommas(data.category5));
									}else{
										$("#category5").text('0');
									}
									if(data.category6!=undefined){// 상품플러스
										$("#category6").text(tspTools.addCommas(data.category6));
											if(data.category61!=undefined){
												$("#category61").text(tspTools.addCommas(data.category61));
											}else{
												$("#category61").text('0')
											}
											if(data.category62!=undefined){
												$("#category62").text(tspTools.addCommas(data.category62));
											}else{
												$("#category62").text('0');
											}
									}else{
									...
									if(data.category7!=undefined){// 은행자산관리서양식
								...
								$("#category0").text('0');
								$("#category1").text('0');$("#category11").text('0');
								$("#category12").text('0');	$("#category13").text('0');
								$("#category14").text('0');$("#category2").text('0');
								$("#category21").text('0');	$("#category22").text('0');
								$("#category3").text('0');$("#category31").text('0');
								$("#category32").text('0');$("#category4").text('0');
								$("#category5").text('0');$("#category6").text('0');
								$("#category61").text('0');	$("#category62").text('0');
								$("#category7").text('0');
								$("#rightClusterDiv").hide();//전체검색결과가 없으면오른쪽 클러스터링 쪽 영역 보여주지 않기
								
							}
							
							if($("#timeLineDiv").css('display')!='none'){
								timeLineSetData("flash",data.timeLineResult);
	...
	//검색메소드별로 subCategory 코드값 알아내기
	// 서브카테고리 
	var searchMethodType=["all","newTurn","kms_map","kms_qna","kms_faq","kms_wiki","hanaPortal","approval",
			"gwboard","guideRule","rule","guide","hanaTv","newTurnCoP","prodPlus","prodInfo","prodBoard","bankform"];
	var searchMethodCode=["0","1","11","12","13","14","2","21",
	          			"22","3","31","32","41","51","6","61","61","7"];
