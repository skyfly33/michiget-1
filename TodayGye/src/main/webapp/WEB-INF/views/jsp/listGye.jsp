			...
			<div 
				...
				<ul>
					<c:if test="${page != 1 && page != 0}">
						<a href="/todaygye/gye/listAll.do?page=${firstPage}">처음</a>
					</c:if>
					<c:if test="${block != 1 && block !=0}">
						<a href="/todaygye/gye/listAll.do?page=${startPage -1}">이전</a>
					</c:if>
					<c:forEach step="1" begin="${startPage }" end="${endPage }" var="pagePrint">
								...
								<c:choose>
									...
									<a href="/todaygye/gye/listAll.do?page=${pagePrint}"><B>${pagePrint }</B></a>&nbsp;
									</c:when>
									<c:otherwise>
									<a href="/todaygye/gye/listAll.do?page=${pagePrint}">${pagePrint }</a>&nbsp;
									</c:otherwise>
					...
					<c:if test="${block != totalBlock && totalBlock !=0}">
						<a href="/todaygye/gye/listAll.do?page=${endPage +1}">다음</a>
					</c:if>
					<c:if test="${page != pageCnt && pageCnt != 0}">
						<a href="/todaygye/gye/listAll.do?page=${pageCnt}">마지막</a>
					</c:if>
				</ul>
			</div>
